...
class PointNoise():
    ...
    def __call__(self, x):
        ...
        noise = noise = np.random.normal(0, self.std, (x.shape[1], 2))
...
class JointNoise():
    ...
    def __call__(self, x):
        ...
        noise = np.random.normal(0, self.std, (x.shape[1], 2)).astype(np.float32)
        return x + np.repeat(noise[np.newaxis, ...], x.shape[0], axis=0)
...
class MultiInput():
    def __init__(self,if_OU=False):
        if if_OU:
            self.connect_joint=np.array([1,1,1,2,3,1,5,6,2,8,9,5,11,12,0,0,14,15])
        else:
            self.connect_joint = np.array([0,0,0,1,2,0,0,5,6,7,8,5,6,11,12,13,14])

    def __call__(self, data):

        data = data.transpose((2,0,1))  #tvc->ctv
        C, T, V = data.shape
        ...
        for i in range(V):
            bone[:, :, i] = data[:, :, i] - data[:, :, self.connect_joint[i]]
        inp = np.concatenate((joint, velocity, bone),axis = 0) #ctv
        inp = inp.transpose((1,2,0)) #tvc
        return inp  #tvc
# ******************************************
