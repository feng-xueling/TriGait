...
def main(cfgs,Training):
    ...
    '''
    target_layers = [model.ES.ConvA3[1]]  # 定义目标层
    ...
    for i in range(sil.shape[0]):
        visualization.append(show_cam_on_image(sil[i,:].data.cpu().numpy().astype(dtype=np.float32),
                                      grayscale_cam[i,:],
                                      use_rgb=True))
    for i,vi in enumerate(visualization[:10]):
        plt.subplot(2,5,i+1),plt.imshow(vi)

    print("===========saveconv============")


    plt.savefig('backbone.jpg')

    plt.close()


    target_layers= [model.attention_s.global_SpatialGate]
    cam = GradCAM2d(model=model, target_layers=target_layers, use_cuda=False)
    # target_category = 254  # pug, pug-dog
    sil = ipts[0][0, :].mean(0)
    ...
    visualization = show_cam_on_image(sil.data.cpu().numpy().astype(dtype=np.float32),
                                      grayscale_cam,
                                      use_rgb=True)
    ...
    target_layers = [model.attention_s.local_SpatialGate0,model.attention_s.local_SpatialGate1,model.attention_s.local_SpatialGate2,model.attention_s.local_SpatialGate3
                     ,model.attention_s.local_SpatialGate4,model.attention_s.local_SpatialGate5,model.attention_s.local_SpatialGate6,model.attention_s.local_SpatialGate7]
    cam = GradCAMlocal(model=model, target_layers=target_layers, use_cuda=False)
    # target_category = 254  # pug, pug-dog
    sil = ipts[0][0, :].mean(0)
