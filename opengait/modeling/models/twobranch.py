...
class TwoBranch(BaseModel):
    def __init__(self, *args, **kargs):
        super(TwoBranch, self).__init__(*args, **kargs)
    ...
    def build_network(self, model_cfg):
        ...
        if dataset_name in ['CASIA-B']:
            self.ES = backbone_3d_ca(sil_channels)
        else:
            self.ES = backbone_3d(sil_channels)
        ...
        if 'SeparateBNNecks' in model_cfg.keys():
            ...
            self.Head1 = SeparateFCs(model_cfg["bin_num"][0], model_cfg['hidden_dim'], model_cfg['class_num'])
            self.Bn_head = True'''
        self.FCs = SeparateFCs(**model_cfg['SeparateFCs'])
        self.Bn = nn.BatchNorm1d(model_cfg['hidden_dim'])
        self.Head1 = SeparateFCs(81, model_cfg['hidden_dim'], model_cfg['class_num'])
    ...
    def forward(self, inputs):
        ...
        fuse = self.cat_s_p(outs,x)
        #fuse = torch.cat([outs, x], 1)


        gait = self.FCs(fuse)  # [n, c, p]
