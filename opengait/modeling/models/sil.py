...
class GLSpatialGate(nn.Module):
    def __init__(self, gate_channel, having=3, fm_sign=False):
...
class SIL(BaseModel):
    ...
    def build_network(self, model_cfg):
        ...
        self.attention_t = TemporalGate(sil_channels[-1], sil_channels[-1],bin_num=model_cfg["bin_num"])#,model_cfg['bin_num'])
        ...
        self.Head1 = SeparateFCs(64, sil_channels[-1], 74)
        self.Bn = nn.BatchNorm1d(sil_channels[-1])
    ...
    def forward(self, inputs):
        ...
        #f = self.TP(f, seqL, options={"dim": 2})[0]
        #f = self.HPP(f)
        #outs = f_pa
        ...
        embed_2 = self.Bn(embed_1)  # [n, c, p]
        logi = self.Head1(embed_2)  # [n, c, p]
        embed = embed_2
