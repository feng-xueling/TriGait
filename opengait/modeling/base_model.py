...
class BaseModel(MetaModel, nn.Module):
    ...
    def run_featuremap(model):
        ...
        if rank == 0:
            ...
            info_dict.update({
                'labels': label_list, 'types': types_list, 'views': views_list})
            feature, fuse, label, seq_type, view = info_dict['embeddings'],info_dict['logits'], info_dict['labels'], info_dict['types'], info_dict['views']

            n,_,_ = feature.shape


            feature_bank = np.mean(feature,2)+np.max(feature,2) #feature.reshape(n,-1)
            tsne = TSNE(2)
            ...
            label_bank = label
            # 带真实值类别
            label_bank = [int(num) % 100 for num in label_bank]
            ...
            for i in range(81,91):  # 对每类的数据画上特定颜色的点
                index = (label_bank == i)
                print(index)
                plt.scatter(output[index, 0], output[index, 1], s=5, cmap=plt.cm.Spectral)
                #ax.scatter(output[index, 0], output[index, 1],output[index, 2], cmap=plt.cm.Spectral)
